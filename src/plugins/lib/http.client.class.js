...
class HttpBox extends LocationHelper {
  ...
  async get(url, config) {
    try {
      ...
      if (isDebug) console.log(`get.url: "${url}"; get.config: ${_config}`)
      const response = await axios.get(url, _config)
      if (isDebug) console.log('get.response:', response)
      if (response.statusText !== 'OK') {
        throw new Error(
          `HttpBox.get Error: Network response was not OK; url: '${url}'; config: `,
          _config
        )
      }
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new TypeError(
          `HttpBox.get Error:  we haven't got JSON;  url: '${url}'; config: `,
          _config
        )
      }
    } catch (ex) {
  ...
  async post(url, data, config) {
    try {
      ...
      if (isDebug)
        console.log(
          `post.url: "${url}"; post.config: ${_config}; post.data: ${data}`
        )
      const response = await axios.post(url, data, _config)
      if (isDebug) console.log('post.response:', response)
      if (response.statusText !== 'Created') {
        throw new Error(
          `HttpBox.post Error: Network response was not OK; url: '${url}'; config: `,
          _config
        )
      }
      const contentType = response.headers['content-type']
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new TypeError(
          `HttpBox.post Error:  we haven't got JSON;  url: '${url}'; config: `,
          _config
        )
      }
    } catch (ex) {
  ...
  async put(url, data, config) {
    try {
      ...
      if (isDebug)
        console.log(
          `put.url: "${url}"; put.config: ${_config}; put.data: ${data}`
        )
      const response = await axios.put(url, data, _config)
      if (isDebug) console.log('put.response:', response)
      if (response.statusText !== 'OK') {
        throw new Error(
          `HttpBox.put Error: Network response was not OK; url: '${url}'; config: `,
          _config
        )
      }
      const contentType = response.headers['content-type']
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new TypeError(
          `HttpBox.put Error:  we haven't got JSON;  url: '${url}'; config: `,
          _config
        )
      }
    } catch (ex) {
  ...
  async patch(url, data, config) {
    try {
      ...
      if (isDebug)
        console.log(
          `patch.url: "${url}"; patch.config: ${_config}; patch.data: ${data}`
        )
      const response = await axios.patch(url, data, _config)
      if (isDebug) console.log('patch.response:', response)
      if (response.statusText !== 'OK') {
        throw new Error(
          `HttpBox.patch Error: Network response was not OK; url: '${url}'; config: `,
          _config
        )
      }
      const contentType = response.headers['content-type']
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new TypeError(
          `HttpBox.patch Error:  we haven't got JSON;  url: '${url}'; config: `,
          _config
        )
      }
    } catch (ex) {
  ...
  async delete(url, config) {
    try {
      ...
      const _config = Object.assign(configDefault, config)
      if (isDebug)
        console.log(`delete.url: "${url}"; delete.config: ${_config}`)
      const response = await axios.delete(url, _config)
      if (isDebug) console.log('delete.response:', response)
      if (response.statusText !== 'OK') {
        throw new Error(
          `HttpBox.delete Error: Network response was not OK; url: '${url}'; config: `,
          _config
        )
      }
      const contentType = response.headers['content-type']
      if (contentType && contentType.includes('application/json')) {
        ...
        throw new TypeError(
          `HttpBox.delete Error:  we haven't got JSON;  url: '${url}'; config: `,
          _config
        )
      }
    } catch (ex) {
