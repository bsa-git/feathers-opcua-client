...
class HookHelper {
  ...
  constructor(context) {
    ...
    this.contextUser =
      this.contextParams && this.contextParams.user
        ? this.contextParams.user
        : null
    // Get context.params.authenticated
    this.contextAuthenticated =
      this.contextParams && this.contextParams.authenticated
        ? this.contextParams.authenticated
        : null
    // Get contextParams.payload
    this.contextPayload =
      this.contextParams && this.contextParams.payload
        ? this.contextParams.payload
        : null
    ...
    this.contextAccessToken =
      this.contextResult && this.contextResult.accessToken
        ? this.contextResult.accessToken
        : ''
    ...
    this.contextProvider =
      this.contextParams && this.contextParams.provider
        ? this.contextParams.provider
        : ''
  ...
  isMask(mask = '') {
    ...
    return (
      maskItems[0] === this.contextPath &&
      maskItems[1] === this.contextMethod &&
      maskItems[2] === this.contextType
    )
  }
  ...
  showDebugError() {
    if (this.contextError) {
      ...
      debug(`ErrorMessage: ${this.contextError.message}`, _contextError)
    }
  }
  ...
  getDebugError() {
    ...
    if (this.contextError) {
      ...
      _contextError.service = _contextError.hook
        ? `${_contextError.hook.path}.${_contextError.hook.method}.${_contextError.hook.type}`
        : 'error'
      ...
      if (_contextError.data && !Object.keys(_contextError.data).length)
        delete _contextError.data
      if (_contextError.errors && !Object.keys(_contextError.errors).length)
        delete _contextError.errors
  ...
  static getIdField(items) {
    ...
    return idField
      ? idField
      : new errors.GeneralError('Items argument is not an array or object')
  }
  ...
  getPickRecords(fn) {
    let _records
    if (Array.isArray(this.contextRecords)) {
      _records = this.contextRecords.map(record =>
        fn(Object.assign({}, record))
      )
    } else {
